...
;(async () => {
  try {
    ...
    for (const containment of cases) {
      ...
      const create = await request('/defects', {
        ...
        body: JSON.stringify({
          ...
          description: `E2E test defect for qty_on_hold (${containment})`,
          qty_affected: 7,
      ...
      console.log('\nDefect created id:', defect.id, 'containment_status:', containment, 'qty_on_hold at creation:', defect.qty_on_hold)
      ...
      console.log('Corrective action fetched. action.qty_affected=', actionData.qty_affected, 'qty_on_hold stored=', actionData.qty_on_hold, 'defect_containment_status=', actionData.defect_containment_status)

      // Compute what the frontend shows before any edits (all quantities zero)
      const affected = Number(actionData.qty_affected || 0)
      const computedOnHold = actionData.defect_containment_status === 'No Hold Needed'
        ? 0
        : Math.max(0, affected - (0 + 0 + 0 + 0 + 0))
      console.log('Computed qty_on_hold on CA form before submit:', computedOnHold)
